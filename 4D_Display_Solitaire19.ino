//Solitaire_v19.  Arduino RBT using 4D SYSTEMS 4.3” Serial Display Module uLCD-43-DCT
...
{   
 state = Display.touch_Get(TOUCH_STATUS);                  // get touchscreen status
     if (state == TOUCH_PRESSED)                           // if there"s a touch
         {
         x = Display.touch_Get(TOUCH_GETX);                // 480
         y = Display.touch_Get(TOUCH_GETY);                // 272
  ...
  switch (Selected){
      case 2:   // If two tiles are selected.
                Count = Count + 1;  // Increment player score.
                Selected = 0;
                delay(1500);  // Display 2nd tile for a second and a half.
    ...
    Display.print(Count);  // Display player score.
}
